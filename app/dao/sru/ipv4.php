...
extends UFdao {
	...
	public function getFreeByDormitoryIdAndVlan($dormitory, $vlan) {
	...
	public function getUsedByDorm($dormId) {
		...
		$query = $this->prepareSelect($mapping);
		$query->where('i.dormitory_id = '.$dormId. ' and i.ip=c.ipv4 and c.active = true AND i.vlan = '.UFbean_SruAdmin_Vlan::getDefaultVlan(), null, $query->SQL);
	...
	public function getSumByDorm($dormId) {
		...
		$query = $this->prepareSelect($mapping);
		$query->where('i.dormitory_id = '.$dormId.' AND i.vlan = '.UFbean_SruAdmin_Vlan::getDefaultVlan(), null, $query->SQL);
