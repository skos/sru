ALTER TABLE users ADD COLUMN last_location_change timestamp without time zone;
COMMENT ON COLUMN users.last_location_change IS 'data ostatniej zmiany lokalizacji';
ALTER TABLE users_history ADD COLUMN last_location_change timestamp without time zone;
COMMENT ON COLUMN users_history.last_location_change IS 'data ostatniej zmiany lokalizacji';
...
then
	...
	(OLD.last_location_change IS NULL AND NEW.last_location_change IS NOT NULL)
then
	INSERT INTO users_history (
		user_id,
		name,
