...
extends UFlib_ClassWithService {
	...
	public function edit() {
		...
		$enabled = $conf->puuAvailable;
		if (!$enabled) {
			return false;
		}
		$user = UFra::factory('UFbean_Sru_User');
		$user->getFromSession();
		if ($user->servicesAvailable && !$user->banned && ($user->typeId < UFbean_Sru_User::DB_STUDENT_MAX || $user->typeId == UFbean_Sru_User::TYPE_TOURIST_STUDENT || $user->typeId == UFbean_Sru_User::TYPE_TOURIST_DIDACTICS || $user->typeId == UFbean_Sru_User::TYPE_ORGANIZATION || $user->typeId == UFbean_Sru_User::TYPE_EXADMIN)) {
			return true;
		}
		return false;
	}
