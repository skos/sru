...
extends UFlib_Snmp {
	...
	public $movedTrunksNumbersDown = array(
		'J8697A',
	);
	...
	public function setPortStatus($port, $status) {
		...
		$portStatus = $this->getPortStatus($port);
		if (($status == self::DISABLED && $portStatus == self::DISABLED) || ($status != self::DISABLED && $portStatus != self::DISABLED)) {
			return false;
		}
			...
			if ($portUser) {
				...
				try {
					if ($portUser > $switch->modelPorts) {
						...
						if (in_array($this->switch->modelNo, $this->movedTrunksNumbersUp)) {
							$name = $name - 2;
						} else if (in_array($this->switch->modelNo, $this->movedTrunksNumbersDown)) {
							$name = $name + 2;
						}
